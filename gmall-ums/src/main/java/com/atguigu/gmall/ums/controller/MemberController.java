...
public class MemberController {
    ...
    public Resp<Boolean> checkout(@PathVariable("data")String data,@PathVariable("type")Integer type){
        Boolean b=memberService.checkout(data,type);
        return Resp.ok(b);
    }

    @GetMapping("query")
    public Resp<MemberEntity> queryMemberEntity(@PathParam("username")String username,@PathParam("password")String password){
        MemberEntity memberEntity=this.memberService.queryMemberEntity(username,password);

        return Resp.ok(memberEntity);
    }
    @PostMapping("register")
    public Resp<Object> register(MemberEntity memberEntity , @PathParam("code")String code){
